...
public class TracingFilter extends Filter {
  ...
  public CompletionStage<Result> apply(Function<Http.RequestHeader, CompletionStage<Result>> next, Http.RequestHeader request) {
    ...
    if (Objects.nonNull(def)) {
      ...
      Http.RequestHeader newRequest = request.addAttr(Helper.KEY_SNAPSHOT, snapshot);
      span.tag(new StringTag("request.attrs"), newRequest.attrs().toString());

      span.prepareForAsync();
      CompletionStage<Result> stage = next.apply(newRequest).thenApply(result -> {
